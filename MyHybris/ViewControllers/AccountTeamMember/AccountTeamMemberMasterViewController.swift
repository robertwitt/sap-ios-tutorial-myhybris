...
class AccountTeamMemberMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var apimktcorporateaccountsrvEntities: APIMKTCORPORATEACCOUNTSRVEntities<OnlineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([AccountTeamMember]?, Error?) -> Void) -> Void)?
    private var entities: [AccountTeamMember] = [AccountTeamMember]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let accountteammember = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CorporateAccountOrigin, CorporateAccountID, TeamMemberID, Role", value: "\(accountteammember.corporateAccountOrigin!), \(accountteammember.corporateAccountID!), \(accountteammember.teamMemberID!), \(accountteammember.role!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! AccountTeamMemberDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].corporateAccountOrigin ?? ""
...
extension AccountTeamMemberMasterViewController: EntitySetUpdaterDelegate {
