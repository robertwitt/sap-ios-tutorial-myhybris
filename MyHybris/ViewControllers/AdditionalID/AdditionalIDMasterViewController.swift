...
class AdditionalIDMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var apimktcorporateaccountsrvEntities: APIMKTCORPORATEACCOUNTSRVEntities<OnlineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([AdditionalID]?, Error?) -> Void) -> Void)?
    private var entities: [AdditionalID] = [AdditionalID]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let additionalid = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CorporateAccountOrigin, CorporateAccountID, InteractionContactAdditionalOrigin, InteractionContactAdditionalExternalID", value: "\(additionalid.corporateAccountOrigin!), \(additionalid.corporateAccountID!), \(additionalid.interactionContactAdditionalOrigin!), \(additionalid.interactionContactAdditionalExternalID!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! AdditionalIDDetailViewController
...
extension AdditionalIDMasterViewController: EntitySetUpdaterDelegate {
