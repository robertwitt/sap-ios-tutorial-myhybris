...
class MarketingAreaMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var apimktcorporateaccountsrvEntities: APIMKTCORPORATEACCOUNTSRVEntities<OnlineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([MarketingArea]?, Error?) -> Void) -> Void)?
    private var entities: [MarketingArea] = [MarketingArea]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let marketingarea = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CorporateAccountOrigin, CorporateAccountID, InteractionContactMktgArea", value: "\(marketingarea.corporateAccountOrigin!), \(marketingarea.corporateAccountID!), \(marketingarea.interactionContactMktgArea!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! MarketingAreaDetailViewController
...
extension MarketingAreaMasterViewController: EntitySetUpdaterDelegate {
