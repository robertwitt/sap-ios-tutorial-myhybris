...
class MarketingAttributeMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var apimktcorporateaccountsrvEntities: APIMKTCORPORATEACCOUNTSRVEntities<OnlineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([MarketingAttribute]?, Error?) -> Void) -> Void)?
    private var entities: [MarketingAttribute] = [MarketingAttribute]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let marketingattribute = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CorporateAccountOrigin, CorporateAccountID, MarketingAttributeCategory, MarketingAttributeValue", value: "\(marketingattribute.corporateAccountOrigin!), \(marketingattribute.corporateAccountID!), \(marketingattribute.marketingAttributeCategory!), \(marketingattribute.marketingAttributeValue!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! MarketingAttributeDetailViewController
...
extension MarketingAttributeMasterViewController: EntitySetUpdaterDelegate {
