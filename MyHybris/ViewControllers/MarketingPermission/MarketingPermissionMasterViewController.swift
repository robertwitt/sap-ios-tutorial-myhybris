...
class MarketingPermissionMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var apimktcorporateaccountsrvEntities: APIMKTCORPORATEACCOUNTSRVEntities<OnlineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([MarketingPermission]?, Error?) -> Void) -> Void)?
    private var entities: [MarketingPermission] = [MarketingPermission]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let marketingpermission = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CorporateAccountID, CorporateAccountOrigin, CorporateAccountPermissionID, CorpAcctPermissionOrigin, MarketingArea, CommunicationMedium, CommunicationCategory", value: "\(marketingpermission.corporateAccountID!), \(marketingpermission.corporateAccountOrigin!), \(marketingpermission.corporateAccountPermissionID!), \(marketingpermission.corpAcctPermissionOrigin!), \(marketingpermission.marketingArea!), \(marketingpermission.communicationMedium!), \(marketingpermission.communicationCategory!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! MarketingPermissionDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].corporateAccountID ?? ""
...
extension MarketingPermissionMasterViewController: EntitySetUpdaterDelegate {
